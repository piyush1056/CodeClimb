...
### 2. Autonomous Agentic AI Tutor
Integrated **Google Gemini** with a stateful function-calling loop to act as an elite, autonomous algorithmic teaching assistant.
* **Native Tool Use (Function Calling):** The tutor does not guess errors. It is equipped with a custom `executeCodeInSandbox` declaration, allowing the model to autonomously pause its reasoning loop and execute user-submitted code in the sandbox environment when a bug is presented.
* **Contextual Evaluation & Guardrails:** Grounded with full problem definitions, starting code, and visible/hidden test case boundaries. Engineered prompt-level strict guardrails to prevent the agent from leaking raw hidden test-case inputs/outputs, forcing it to explain conceptual edge cases instead.
* **Data Sanitization Pipeline:** Features a specialized regex parsing and `stripMain` compiler cleanup layer that strips out markdown backticks and duplicate driver code generated by the LLM before sending payload tokens to the execution grid.
* **Deterministic Schema Enforcement:** Mandates strict MIME-type output configurations (`application/json`) to ensure the agent's finalized post-tool response perfectly maps to a strict structural contract `{"explanation", "approach", "code", "tips"}` for safe UI rendering.
